...
Tell Pablo to check that the Action ran successfully before proceeding. If this repo is zero-friction, the propagation to all 26 repos will fire automatically.
...
If this merge completes all features for a version milestone, bump `VERSION.txt` to that version number (e.g., `v1.1.0`).
